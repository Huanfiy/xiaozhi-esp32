...
#define TAG "XIAOZHI_Ssd1306Display"

LV_FONT_DECLARE(font_awesome_30_1);
static void IRAM_ATTR gpio_isr_handler(void* arg);

XIAOZHI_Ssd1306Display::XIAOZHI_Ssd1306Display(void* i2c_master_handle, int width, int height, bool mirror_x,
                                               bool mirror_y, const lv_font_t* text_font, const lv_font_t* icon_font)
    ...
    // 创建充电检测定时器
    esp_timer_create_args_t charging_timer_args = {.callback = &XIAOZHI_Ssd1306Display::ChargingTimerCallback,
                                                   .arg = this,
    ...
    // 创建电量检测定时器
    esp_timer_create_args_t battery_timer_args = {.callback = &XIAOZHI_Ssd1306Display::BatteryTimerCallback,
                                                  .arg = this,
                                                  .dispatch_method = ESP_TIMER_TASK,
                                                  .name = "battery_timer"};
    ...
    gpio_set_level(GPIO_NUM_9, 1);  // 使能电源
    ...
    gpio_isr_handler_add(GPIO_NUM_8, gpio_isr_handler, this);
    ...
    // 初始化充电满电引脚, gpio47
    gpio_config_t io_conf_full = {};
    io_conf_full.intr_type = GPIO_INTR_DISABLE;
    io_conf_full.mode = GPIO_MODE_INPUT;
    io_conf_full.pin_bit_mask = (1ULL << charging_full_pin_);
    io_conf_full.pull_down_en = GPIO_PULLDOWN_DISABLE;
    io_conf_full.pull_up_en = GPIO_PULLUP_ENABLE;
    gpio_config(&io_conf_full);

    xTaskCreate(XIAOZHI_Ssd1306Display::user_custom_task, "user_custom_task", 2048, this, 5, NULL);
    /*===============================================*/

    boot_button_.OnPressDown([this]() { this->UpdateInteractionTime(); });

    volume_up_button_.OnPressDown([this]() { this->UpdateInteractionTime(); });
...
XIAOZHI_Ssd1306Display::~XIAOZHI_Ssd1306Display() {
    if (content_ != nullptr) {
        lv_obj_del(content_);
    }
...
void XIAOZHI_Ssd1306Display::UpdateInteractionTime() {
    last_interaction_time_ = esp_timer_get_time();
    if (is_light_sleep_) {
        // 从浅睡眠中唤醒，打开显示
...
void XIAOZHI_Ssd1306Display::CheckSleepState() {
...
bool XIAOZHI_Ssd1306Display::Lock(int timeout_ms) {
    return lvgl_port_lock(timeout_ms);
}

void XIAOZHI_Ssd1306Display::Unlock() {
    lvgl_port_unlock();
}

uint16_t XIAOZHI_Ssd1306Display::ReadBatteryLevel() {
...
void XIAOZHI_Ssd1306Display::BatteryTimerCallback(void* arg) {
    XIAOZHI_Ssd1306Display* display = static_cast<XIAOZHI_Ssd1306Display*>(arg);
...
void XIAOZHI_Ssd1306Display::StartChargingTimer() {
    ESP_ERROR_CHECK(esp_timer_start_periodic(charging_timer_, adc_samp_interval));
}

void XIAOZHI_Ssd1306Display::StartBatteryTimer() {
    ESP_ERROR_CHECK(esp_timer_start_periodic(battery_timer_, adc_samp_interval));
}

void XIAOZHI_Ssd1306Display::SetChatMessage(const char* role, const char* content) {
...
void XIAOZHI_Ssd1306Display::SetupUI_128x64() {
    DisplayLockGuard lock(this);
...
void XIAOZHI_Ssd1306Display::SetupUI_128x32() {
    DisplayLockGuard lock(this);
...
void XIAOZHI_Ssd1306Display::UpdateBatteryAndChargingDisplay(uint16_t average_adc) {
    DisplayLockGuard lock(this);
...
void XIAOZHI_Ssd1306Display::ChargingTimerCallback(void* arg) {
    XIAOZHI_Ssd1306Display* display = static_cast<XIAOZHI_Ssd1306Display*>(arg);
...
void XIAOZHI_Ssd1306Display::ShowLowBatteryPopup() {
    DisplayLockGuard lock(this);
...
void XIAOZHI_Ssd1306Display::OnStateChanged() {
...
void XIAOZHI_Ssd1306Display::show_power_off_popup() {
    DisplayLockGuard lock(this);
    if (power_off_popup_ == nullptr) {
        // 创建弹出窗口
        power_off_popup_ = lv_obj_create(lv_scr_act());
        lv_obj_set_size(power_off_popup_, 120, 30);
        lv_obj_center(power_off_popup_);
        lv_obj_set_style_bg_color(power_off_popup_, lv_color_black(), 0);
        lv_obj_set_style_radius(power_off_popup_, 10, 0);

        // 创建提示文本标签
        lv_obj_t* label = lv_label_create(power_off_popup_);
        lv_label_set_text(label, "电源即将关闭");
        lv_obj_set_style_text_color(label, lv_color_white(), 0);
        lv_obj_center(label);
    }
}

void XIAOZHI_Ssd1306Display::clear_power_off_popup() {
    DisplayLockGuard lock(this);
    // 不是隐藏，而是删除
    if (power_off_popup_ != nullptr) {
        lv_obj_del(power_off_popup_);
        power_off_popup_ = nullptr;
    }
}


void XIAOZHI_Ssd1306Display::user_custom_task(void* arg) {
    // 将参数转换为类实例指针
    XIAOZHI_Ssd1306Display* display = static_cast<XIAOZHI_Ssd1306Display*>(arg);
    while (1) {
        if (xSemaphoreTake(display->gpio_sem, portMAX_DELAY) == pdTRUE) {
            int count = 0;
            for (int i = 0; i < display->power_off_time / 100; i++) {
                if (gpio_get_level(GPIO_NUM_8) == 0) {
                    count++;
                    display->show_power_off_popup();
                    vTaskDelay(pdMS_TO_TICKS(100));  // 每100ms检查一次
                } else {
                    if (display->power_off_popup_ != nullptr) {
                        display->clear_power_off_popup();
                    }
                    count = 0;
                    break;
                }
            ...
            if (count >= display->power_off_time / 100) {
                count = 0;
                gpio_set_level(GPIO_NUM_9, 0);
                esp_lcd_panel_disp_on_off(display->panel_, false);
                esp_deep_sleep_start();
            } else {
                if (display->power_off_popup_ != nullptr) {
                    display->clear_power_off_popup();
...
static void IRAM_ATTR gpio_isr_handler(void* arg) {
    ...
    xSemaphoreGiveFromISR(display->gpio_sem, &xHigherPriorityTaskWoken);
