...
class XIAOZHI_Ssd1306Display : public Display {
private:
    ...
    lv_obj_t* power_off_popup_ = nullptr;    // 电源关闭提示窗口对象指针
    int32_t adc_samp_interval = 500000;      // adc值采样的时间间隔，单位为微秒
    const int power_off_time = 2000;
    uint16_t average_adc = 0;                     // adc平均值
    esp_timer_handle_t charging_timer_;           // 充电检测定时器句柄
    esp_timer_handle_t battery_timer_;            // 电量检测定时器句柄
    gpio_num_t charging_pin_ = GPIO_NUM_21;       // 充电检测引脚
    gpio_num_t charging_full_pin_ = GPIO_NUM_47;  // 充电满电检测引脚
    ...
    void show_power_off_popup();                  // 显示电源关闭提示窗口的方法声明
    void clear_power_off_popup();                 // 清除电源关闭提示窗口的方法声明
...
public:
    ...
    XIAOZHI_Ssd1306Display(void* i2c_master_handle, int width, int height, bool mirror_x, bool mirror_y,
                           const lv_font_t* text_font, const lv_font_t* icon_font);
    ~XIAOZHI_Ssd1306Display();
    ...
    static void user_custom_task(void* arg);
